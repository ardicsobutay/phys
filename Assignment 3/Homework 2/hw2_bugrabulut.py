...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while guessLeft > 0 and not isWordGuessed(secretWord, lettersGuessed):
        print "~~~~~~~~"
        print "You have %s guessed left" %guessLeft
        print "Available letters: %s" %getAvailableLetters(lettersGuessed)
        letterEntered = raw_input("Please guess a letter: ").lower()

        if len(letterEntered) == 1:
            
            if letterEntered not in alphabet:
...
hangman(secretWord = chooseWord(wordlist).lower())
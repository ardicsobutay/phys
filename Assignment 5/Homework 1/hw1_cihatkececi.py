...
def buildCoder(shift):
    ...
    """
    shiftedLetters = {}
    elnum = len(string.ascii_lowercase)
    
    for i in range(0, elnum):
       shiftedLetters[string.ascii_uppercase[i]] = string.ascii_uppercase[(i + shift) % elnum]
       
    for i in range(0, elnum):
       shiftedLetters[string.ascii_lowercase[i]] = string.ascii_lowercase[(i + shift) % elnum]
...
def findBestShift(wordList, text):
    ...
    for i in range(0, 26):
        ...
        numValidWords = 0
        for st in shiftedText.split(' '):
            if isWord(wordList, st):
                numValidWords += 1
                
        if numValidWords > maxWordsFound:
            maxWordsFound = numValidWords
            bestShift = i

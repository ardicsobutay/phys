...
def updateHand(hand, word):
    ...
    for letter,repeat in  getFrequencyDict(word).iteritems(): 
        copy_hand[letter] = copy_hand[letter] - repeat
    return copy_hand
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for letter,number in getFrequencyDict(word).iteritems():
            if letter not in hand :
                return False   
            elif number > hand[letter] : 
                return  False         
        else:
            return  True    
    else:
...
def calculateHandlen(hand):
    ...
    """
    sums = 0
    for letter,number in hand.iteritems():
        sums = sums + number
    return sums
